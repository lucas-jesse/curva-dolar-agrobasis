...
def gerar_tickers_dol(data_base, meses=36):
    ...
    tickers = []
    for d in datas:
        tickers.append(f"BMFBOVESPA:DOL{MONTH_CODES[int(d.month)]}{int(d.year)}")
    return tickers
...
def fetch_dol_tradingview_contracts(data_base_str: str, meses_busca: int = 36):
    """
    Busca contratos individuais do DOL na API pública do screener do TradingView.
    A página de contratos do TradingView lista os vencimentos DOL; aqui consultamos os símbolos
    individuais gerados pelo código de mês da B3 e usamos o fechamento/último preço disponível.
    """
    data_base = pd.Timestamp(data_base_str)
    tickers = gerar_tickers_dol(data_base, meses_busca)
    url = "https://scanner.tradingview.com/brazil/scan"
    payload = {
        "symbols": {"tickers": tickers, "query": {"types": []}},
        "columns": ["name", "description", "close", "change", "volume", "open_interest"],
    ...
    try:
        ...
        for item in r.json().get("data", []):
            ...
            d = item.get("d", [])
            close = d[2] if len(d) > 2 else np.nan
            preco = normalizar_preco_dol(close)
            venc = vencimento_dol_para_data(symbol)
            if pd.notna(venc) and np.isfinite(preco) and 3.0 <= preco <= 8.0:
                rows.append({
                    "Contrato": symbol,
                    "vencimento": venc,
                    "Dólar Futuro": preco,
                    "Volume": d[4] if len(d) > 4 else np.nan,
                    "Open Interest": d[5] if len(d) > 5 else np.nan,
                })
        df = pd.DataFrame(rows).drop_duplicates(subset=["Contrato"]).sort_values("vencimento")
        df = df[df["vencimento"] >= pd.Timestamp(data_base).normalize()]
        if len(df) >= 2:
            df.attrs["fonte"] = "TradingView · contratos DOL B3"
            return df.reset_index(drop=True), None
        return pd.DataFrame(), "TradingView não retornou contratos DOL suficientes"
    except Exception as e:
        return pd.DataFrame(), str(e)
